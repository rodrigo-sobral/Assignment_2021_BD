...
exports.getAllRunningAuctions= async () => { 
    const all_elections= await data_management.searchSpecificAuction('leilaoId, titulo, artigo_artigoid, descricao, limite', 'fechado', false, false); 
    const artigos = await this.getAllArtigos();
    for (const i in all_elections) {
        if (!await this.checkAuctionLimit(all_elections[i])){ 
            delete all_elections[i];
        }
        else{
             delete all_elections[i]['limite'];
             console.log(artigos);
             for(const j in artigos){
                if (artigos[j]["artigoid"] == all_elections[i]["artigo_artigoid"]){
                    all_elections[i]["artigo_artigoid"] = artigos[j]["nome_artigo"];
                } 
             }
        }
    } 
...
exports.getAuctionByArtigoidOrDescricao= async (keyword) => {
    ...
    try {
        ...
        let data = await data_management.getAuctionByArtigoidOrDescricao(artigoId);
        for (const i in data) { 
            for(const j in artigos){
                if (artigos[j]["artigoid"] == data[i]["artigo_artigoid"]){
                    data[i]["artigo_artigoid"] = artigos[j]["nome_artigo"];
                } 
            }  
        } 
        return data;
    } catch (e) { let data = await data_management.getAuctionByArtigoidOrDescricao(keyword);
        for (const i in data) { 
            for(const j in artigos){
                if (artigos[j]["artigoid"] == data[i]["artigo_artigoid"]){
                    data[i]["artigo_artigoid"] = artigos[j]["nome_artigo"];
...
exports.getAllInfoAboutSpecificAution= async (leilaoId) => {
    ...
    let author = await data_management.searchSpecificUser("username","userid",selected_auction["users_userid"]);
    selected_auction['autor'] = author[Object.keys(author)[0]]["username"];
    const artigos = await this.getAllArtigos();
    for(const j in artigos){
        if (artigos[j]["artigoid"] == selected_auction["artigo_artigoid"]){
            selected_auction["artigo"] = artigos[j]["nome_artigo"];
        } 
    }
