...
exports.getAuctionByArtigoidOrDescricao= async (parameter_value) => {
	let auctions;
	await database.query('begin transaction isolation level read committed');
	if (typeof parameter_value!=='string') auctions= await database.query(`select leilaoId, titulo, descricao, artigo_artigoid from leilao where artigo_artigoid= ${parameter_value} and fechado=false`);
...
exports.getAllBidsOf= async (what_to_get, parameter, parameter_value) => {
	await database.query('begin transaction isolation level read committed');
	const bid= await database.query(`select username, ${what_to_get} valor_licitado from licitacao, users where ${parameter}= ${parameter_value} and users_userId = userid`);
	await database.query('commit');
	return bid;
}
...
exports.getAllPostsOfAuction= async (leilaoId) => { 
	await database.query('begin transaction isolation level read committed');
	const messages= await database.query(`select users_userid, username, mensagem from mural, users where leilao_leilaoid= ${leilaoId} and users_userId = userid`); 
	await database.query('commit');
	return messages;
}
...
exports.getInboxMessages= async (userId) => { 
	await database.query('begin transaction isolation level read committed');
	const messages= await database.query(`select leilao_leilaoid, titulo, mensagem from inbox, leilao where inbox.users_userid= ${userId} and leilao_leilaoid = leilaoid`);
	await database.query('commit');
	return messages;
}
